...
def create_session(user_uid: str, server_uid: str, ttyrec: str) -> str:
    sid = db_save({
        "type": T_SESSION, "name": f"session-{time.time()}", "user": user_uid, "server": server_uid,
        "started_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "status": "active", "ttyrec_path": ttyrec
    })
    db_save({"uuid": sid, "live_url": f"https://{PUBLIC_VIEWER_HOST}:9000/live/{sid}", "playback_url": f"https://{PUBLIC_VIEWER_HOST}:9000/play_uuid/{sid}"}, sid)
    root = get_root()
...
def create_bootstrap_token(server_uuid: str) -> str:
    ...
    tid = db_save({"type": "bootstrap_token", "server": server_uuid, "expires_at": exp, "used": "false"})
    db_save({"uuid": tid, "token": tid}, tid)
    return tid
