...
Оригинальный размер: 415 строк.
Новый размер: ~130 строк.
"""
import sys, os, argparse, base64, secrets, signal
sys.path.insert(0, os.path.dirname(__file__))

import metax_client_v2 as mx
...
def cmd_group_add(a):
    uid = mx.db_save({"type": T_GROUP, "name": a.name, "members": [], "permissions": []})
    mx.db_save({"uuid": uid}, uid)
    root = mx.get_root(); root.setdefault("groups", []).append(uid); mx.save_root(root)
    print(f"Created group '{a.name}'  UUID: {uid}")
...
def cmd_server_add(a):
    uid = mx.db_save({"type": T_SERVER, "name": a.name, "host": a.host, "port": str(a.port), "bastion_user": a.bastion_user, "bootstrapped": "no"})
    mx.db_save({"uuid": uid}, uid)
    root = mx.get_root(); root.setdefault("servers", []).append(uid); mx.save_root(root)
    print(f"Created server '{a.name}' UUID: {uid}\nRun: python3 pam_cli.py server gen-token --server {uid}")
...
def cmd_perm_add(a):
    uid = mx.db_save({"type": T_PERMISSION, "name": f"perm-{a.group[:8]}-{a.server[:8]}", "group": a.group, "server": a.server, "allow_sudo": "true" if a.sudo else "false"})
    mx.db_save({"uuid": uid}, uid)
